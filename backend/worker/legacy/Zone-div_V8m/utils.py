...
# Ball PNG is picked per job from these env vars (set by jobs.py before spawning
# the pipeline worker). TRACKER_BALL_TYPE selects which coloured ball;
# TRACKER_BALL_ASSETS_DIR overrides where to look for the PNGs (defaults to
# the frontend/public folder shared with the SVG overlay). Falls back to the
# legacy cricket_ball.png in this folder if nothing is set or the file is missing.
_BALL_IMG_CACHE: dict = {}


def _resolve_ball_path() -> str:
    ball_type = os.environ.get("TRACKER_BALL_TYPE", "white").strip().lower()
    assets_dir = os.environ.get("TRACKER_BALL_ASSETS_DIR")
    if assets_dir:
        candidate = os.path.join(
            assets_dir,
            "cricket_ball.png" if ball_type == "white"
            else f"cricket_ball_{ball_type}.png",
        )
        if os.path.isfile(candidate):
            return candidate
    # Fallback: legacy PNG sitting next to this file.
    return os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "cricket_ball.png"
    )
...
def overlay_ball_image(frame: np.ndarray, cx: int, cy: int, size: int = 16) -> np.ndarray:
    ...
    ball_path = _resolve_ball_path()
    key = (ball_path, size)
    if key not in _BALL_IMG_CACHE:
        ball = cv2.imread(ball_path)
        if ball is None:
            return frame
