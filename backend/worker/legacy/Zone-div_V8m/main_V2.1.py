...
# Import sub-modules
from detector import load_model, detect_objects, detect_ball_candidates_low_conf
...
def _mouse_callback(event, x, y, flags, state: _CalibState):
    ...
    if state.active_marker is not None:
...
def run_calibration(first_frame):
    ...
    while True:
        ...
        elif 0 <= key < 128 and chr(key) in KEY_TO_MARKER:
...
def _draw_progressive_overlays(
    ...
    post_bounce_up_to: Optional[int] = None,
    expected_locations: Optional[dict] = None,
) -> "any":
    """
    ...
    # Fallback: no deviation detected → polyline through post-bounce points
    if draw_parabolas and no_deviation and bounce_detector.bounce_point is not None \
            and bounce_to_dev_progress > 0.0:
        _bf, _, _ = bounce_detector.bounce_point
        frame = trajectory.draw_post_bounce_path(
            frame,
            bounce_frame=_bf,
            up_to_frame=post_bounce_up_to,
            progress=bounce_to_dev_progress,
        )
...
# ─────────────────────────────────────────────────────────────────────────────
# FULL-VIDEO RENDER (fallback when release or bounce is missing)
# ─────────────────────────────────────────────────────────────────────────────
def _render_full_video_only(cap, detections_by_frame, pose_detector,
                            release_detector, bounce_detector, trajectory,
                            grid_corners, fps, width, height, total):
    """
    Render every frame with ball bboxes + confidence, pose skeleton, and any
    release/bounce/deviation markers that DID get detected. Used as a graceful
    fallback so the user still sees the detector output even without a full
    release→bounce→deviation timeline.
    """
    ...
    big_fnum = total + 10_000  # sentinel: force all gated markers to draw
    ...
    for fnum in range(total):
        ...
        rel_str = f"Release: F{rel.release_frame}" if rel else "Release: --"
        bp = bounce_detector.bounce_point
        bp_str = f"Bounce: F{bp[0]}" if bp else "Bounce: --"
        balls_in_frame = sum(1 for d in dets if d["class_name"] == "ball")
        status = (f"FULL RENDER | Frame: {fnum} | Ball: {balls_in_frame} "
                  f"| {rel_str} | {bp_str}")
    ...
    out.release()
    logger.close()
    print(f"\n[INFO] Full-render fallback complete.")
...
def process_video():
    ...
    reset_filters()
    bounce_detector  = BounceDetector(calib_markers=calib_markers)
    release_detector = ReleaseDetector(calib_markers=calib_markers)
    trajectory       = Trajectory()
    pose_detector    = PoseDetector()
    pose_detector.set_calibration(calib_markers)

    detections_by_frame: dict[int, list] = {}
    expected_locations_by_frame: dict[int, tuple[float, float]] = {}
    pose_confirmed_at: dict[int, bool] = {}
    cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
    f = 0
    ball_count = 0
    bat_count  = 0
    while True:
        ...
        # Pose gate: release can only be considered after bowler pose confirmed.
        pose_detector.update(frm, f)
        pose_confirmed_at[f] = pose_detector.confirmed

        wrist_xy = pose_detector.get_raised_wrist_coords(f)
        shoulder_xy = pose_detector.get_raised_shoulder_coords(f)
        hand_raise_f = pose_detector.event.frame if pose_detector.confirmed else None
        release_detector.update(
            dets, f,
            pose_confirmed=pose_detector.confirmed,
            wrist_xy=wrist_xy,
            shoulder_xy=shoulder_xy,
            hand_raise_frame=hand_raise_f,
        )
    ...
    if release_detector.release is None:
        print("[WARN] Release point not detected with Pose Gate. Retrying WITHOUT Pose Gate...")
        use_pose_gate = False
        # Reset trackers and detectors
        bounce_detector.reset()
        release_detector = ReleaseDetector(calib_markers=calib_markers)
        trajectory.reset()
        ...
        for f in range(total):
            ...
            hand_raise_f = pose_detector.event.frame if pose_detector.confirmed else None
            release_detector.update(dets, f, pose_confirmed=True, wrist_xy=None, shoulder_xy=None, hand_raise_frame=hand_raise_f)
            rel_f = (release_detector.release.release_frame
                     if release_detector.release else None)
            bounce_detector.update(dets, f, release_frame=rel_f)
            bf = (bounce_detector.bounce_point[0]
    ...
    release_missing = release_detector.release is None
    if release_missing:
        print("[WARN] Release point not detected — will render full video with detections + any available markers.")
        release_f = 0
    else:
        release_f = release_detector.release.release_frame

    # ── Gap Recovery Pass ──
    # Find all frames between release_f and the last ball detection where no ball was detected
    all_ball_frames = sorted([f_idx for f_idx, dets in detections_by_frame.items()
                             if any(d["class_name"] == "ball" for d in dets)])
    if all_ball_frames and not release_missing:
        max_ball_f = all_ball_frames[-1]
        gaps = []
        for f_idx in range(release_f + 1, max_ball_f):
            has_ball = any(d["class_name"] == "ball" for d in detections_by_frame.get(f_idx, []))
            if not has_ball:
                gaps.append(f_idx)
        
        if gaps:
            print(f"[RECOVERY] Found {len(gaps)} potential gap frames between release (F{release_f}) and end of trajectory (F{max_ball_f})...")
            recovered_count = 0
            
            # Re-open capture object to ensure clean seeks
            cap_rec = cv2.VideoCapture(str(VIDEO_PATH))
            if not cap_rec.isOpened():
                print("[RECOVERY] ERROR: Could not open video reader for recovery.")
            else:
                for gf in gaps:
                    prev_candidates = [f for f in all_ball_frames if f < gf]
                    next_candidates = [f for f in all_ball_frames if f > gf]
                    if not prev_candidates or not next_candidates:
                        continue
                    prev_f = max(prev_candidates)
                    next_f = min(next_candidates)
                    
                    prev_ball = next(d for d in detections_by_frame[prev_f] if d["class_name"] == "ball")
                    next_ball = next(d for d in detections_by_frame[next_f] if d["class_name"] == "ball")
                    
                    prev_cx = (prev_ball["x1"] + prev_ball["x2"]) / 2.0
                    prev_cy = (prev_ball["y1"] + prev_ball["y2"]) / 2.0
                    next_cx = (next_ball["x1"] + next_ball["x2"]) / 2.0
                    next_cy = (next_ball["y1"] + next_ball["y2"]) / 2.0
                    
                    # Interpolate expected position
                    t = (gf - prev_f) / (next_f - prev_f)
                    exp_cx = prev_cx + t * (next_cx - prev_cx)
                    exp_cy = prev_cy + t * (next_cy - prev_cy)
                    
                    expected_locations_by_frame[gf] = (exp_cx, exp_cy)
                    
                    cap_rec.set(cv2.CAP_PROP_POS_FRAMES, gf)
                    ret, frm = cap_rec.read()
                    print(f"  [RECOVERY] Frame {gf:3d} read={ret} | Expected: ({int(exp_cx)},{int(exp_cy)})")
                    if ret:
                        # Search with conf = 0.10
                        candidates = detect_ball_candidates_low_conf(session, frm, conf_thresh=0.10)
                        print(f"    Found {len(candidates)} raw ball candidates at conf >= 0.10")
                        for idx, c in enumerate(candidates):
                            cx = (c["x1"] + c["x2"]) / 2.0
                            cy = (c["y1"] + c["y2"]) / 2.0
                            dist = np.sqrt((cx - exp_cx)**2 + (cy - exp_cy)**2)
                            print(f"      Cand {idx}: conf={c['conf']:.3f} at ({int(cx)},{int(cy)}) dist={dist:.1f}px")
                        
                        if candidates:
                            best_cand = min(candidates, key=lambda c: (
                                ((c["x1"] + c["x2"])/2.0 - exp_cx)**2 + 
                                ((c["y1"] + c["y2"])/2.0 - exp_cy)**2
                            ))
                            bc_cx = (best_cand["x1"] + best_cand["x2"]) / 2.0
                            bc_cy = (best_cand["y1"] + best_cand["y2"]) / 2.0
                            dist = np.sqrt((bc_cx - exp_cx)**2 + (bc_cy - exp_cy)**2)
                            
                            if dist < 30.0:  # within 30 pixels
                                print(f"    --> [PROMOTED] Recovered ball at frame {gf} conf={best_cand['conf']:.3f} "
                                      f"dist={dist:.1f}px from expected path")
                                if gf not in detections_by_frame:
                                    detections_by_frame[gf] = []
                                detections_by_frame[gf] = [d for d in detections_by_frame[gf] if d["class_name"] != "ball"]
                                detections_by_frame[gf].append(best_cand)
                                recovered_count += 1
                cap_rec.release()
            
            if recovered_count > 0:
                print(f"[RECOVERY] Successfully recovered {recovered_count} ball detections! Re-running tracking pass...")
                reset_filters()
                bounce_detector.reset()
                release_detector = ReleaseDetector(calib_markers=calib_markers)
                trajectory.reset()
                ...
                f = 0
                while f < total:
                    dets = detections_by_frame.get(f, [])
                    wrist_xy = pose_detector.get_raised_wrist_coords(f) if use_pose_gate else None
                    shoulder_xy = pose_detector.get_raised_shoulder_coords(f) if use_pose_gate else None
                    hand_raise_f = pose_detector.event.frame if (use_pose_gate and pose_detector.confirmed) else None
                    release_detector.update(
                        dets, f,
                        pose_confirmed=True if not use_pose_gate else pose_confirmed_at.get(f, False),
                        wrist_xy=wrist_xy,
                        shoulder_xy=shoulder_xy,
                        hand_raise_frame=hand_raise_f,
                    )
    ...
    # After recovery (and optional re-tracking), extract finalized release/bounce/deviation values
    release_missing = release_detector.release is None
    ...
    # Fallback: if bounce not detected, mark the lowest detection (maximum y) as the bounce point.
    fallback_bounce = None
    if bounce_detector.bounce_point is None and not release_missing:
        print("[WARN] Bounce point not detected within return creases. Falling back to the lowest detection point (max y) post-release...")
        valid_pts = [e for e in bounce_detector._history if not e.interpolated and e.frame >= release_f]
        if valid_pts:
            lowest_pt = max(valid_pts, key=lambda e: e.y)
            by = lowest_pt.y_bottom if lowest_pt.y_bottom else lowest_pt.y
            fallback_bounce = (lowest_pt.frame, lowest_pt.x, by)
            bounce_detector.bounce_point = fallback_bounce
            print(f"[BOUNCE] Fallback detected at frame {lowest_pt.frame} pos=({lowest_pt.x},{by})")

            # Recheck deviations on trajectory since we now have a valid bounce frame!
            trajectory.recheck_deviations(release_f, lowest_pt.frame)

    # Full-render mode: if release or bounce is still missing, render the entire
    # video with detections + pose + any markers we do have, then finish. No
    # segments B/C, no trajectory parabolas — just an honest playback so the
    # user can see what the detector produced.
    full_render_only = release_missing or bounce_detector.bounce_point is None
    if full_render_only:
        print("[INFO] Release or bounce missing — rendering full video with detections + available markers only.")
        _render_full_video_only(
            cap, detections_by_frame, pose_detector,
            release_detector, bounce_detector, trajectory,
            grid_corners, fps, width, height, total,
        )
        return
    no_deviation = not trajectory._dir_changes
    if no_deviation:
        print("[INFO] No deviation detected — falling back to post-bounce path.")

    release_f = release_detector.release.release_frame
    bounce_f  = bounce_detector.bounce_point[0]
    if no_deviation:
        # End the timeline at the last post-bounce ball detection so segments
        # still have a well-defined right edge.
        post_bounce_frames = [f for (f, _, _) in trajectory._pts if f >= bounce_f]
        dev_f = max(post_bounce_frames) if post_bounce_frames else bounce_f
    else:
    ...
    reset_filters()
    bounce_detector  = BounceDetector(calib_markers=calib_markers)
    if fallback_bounce is not None:
        bounce_detector.bounce_point = fallback_bounce
    release_detector = ReleaseDetector(calib_markers=calib_markers)
    trajectory       = Trajectory()
    ...
    for fnum in range(0, seg_a_end + 1):
        ...
        wrist_xy = pose_detector.get_raised_wrist_coords(fnum) if use_pose_gate else None
        shoulder_xy = pose_detector.get_raised_shoulder_coords(fnum) if use_pose_gate else None
        hand_raise_f = pose_detector.event.frame if (use_pose_gate and pose_detector.confirmed) else None
        release_detector.update(
            dets, fnum,
            pose_confirmed=True if not use_pose_gate else pose_confirmed_at.get(fnum, False),
            wrist_xy=wrist_xy,
            shoulder_xy=shoulder_xy,
            hand_raise_frame=hand_raise_f,
        )
        ...
        frame = _draw_progressive_overlays(
            ...
            release_detector, bounce_detector, trajectory,
            draw_parabolas=False,
            expected_locations=expected_locations_by_frame,
        )
    ...
    if no_deviation:
        print("[SEG C] Skipped — no deviation detected, nothing to animate.")
        out.release()
        logger.close()
        frame_count = total
        print(f"\n[INFO] Processing complete (no-deviation fallback).")
        print(f"[INFO] Output video : {VIDEO_OUTPUT}")
        return
